...
from models.cutmix.utils import image_resize, plot_bboxes,read_xml,add_xml
...
def CutmixAugmentation(image_path,xml_path,image_save_path,xml_save_path,category):
    print('------------------------------')
    print('cutmix增强')
    ...
    l1 = read_xml(xml_path,image_name,category)
    l2 = read_xml(xml_path,image_names[i],category)


    # -----------进行cutmix操作---------------
    mix_img, labels = cutmix(img1, l1,img2, l2,)
    img_w,img_h=mix_img.shape[1],mix_img.shape[0]
    add_xml(xml_path,image_name,xml_save_path,labels,folder,image_save_path,img_w,img_h,category)
    final_img = plot_bboxes(mix_img, labels,category)
    # cv2.imshow("output", final_img)  # Show image
    cv2.imwrite(os.path.join(image_save_path,'cutmix_'+image_name),final_img)
    return os.path.join(image_save_path,'cutmix_'+image_name)
...
#     CutmixAugmentation(image_path=img_path, xml_path=xml_path, image_save_path=save_path, xml_save_path=save_xml,category=category)
