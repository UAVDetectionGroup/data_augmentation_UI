...
class SmallObjectAugmentation(object):
    ...
    def create_copy_annot(self, h, w, annot, annots):
        ...
        for epoch in range(self.epochs):
            ...
            # print(annot)
            new_annot = np.array([xmin, ymin, xmax, ymax, annot[4]]).astype(np.int_)
    ...
    def add_patch_in_img(self,copy_annot,annot, img,image):
        ...
        copy_annot = copy_annot.astype(np.int_)
        try:
...
def issmallobject(name,lower_thresh, hight_thresh, h, w,objects):
    ...
    if lower_thresh <= h * w <= hight_thresh :
        if name in objects:
                return True
        else:
            return False
...
def add_xml(xml_path,image_name,xml_save_path,add_object_xml,SMB_image_name,image_save_path,img_h, img_w,category):
    ...
    fp.close()
def collect_small_object(xml_path,Low_SOA_THRESH,SOA_THRESH,category,objects):
    """
    目的：提取图片中所有小目标
    ...
    for xml_name in xmls:
        ...
        for obj in tree1.findall("object"):
            ...
            if issmallobject(name=obj_name, lower_thresh=Low_SOA_THRESH, hight_thresh=SOA_THRESH, h=bbox_h, w=bbox_w,
                             objects=objects):
...
def Samll_object_Augmentation(image_path,xml_path,image_save_path,xml_save_path,SOA_THRESH, SOA_PROB, SOA_COPY_TIMES, SOA_EPOCHS,Low_SOA_THRESH,category,objects):
...
if __name__ == '__main__':
    img_path = r"D:\xiangmu\ui\test\image"  # 原始图片文件夹路径
    ...
    Samll_object_Augmentation(image_path=img_path,xml_path=xml_path,image_save_path=save_path,xml_save_path=save_xml,SOA_THRESH=min_xml,
                              SOA_PROB=1, SOA_COPY_TIMES=3, SOA_EPOCHS=30,Low_SOA_THRESH=max_xml,category=category,objects=objects)
