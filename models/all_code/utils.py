...
def plot_bboxes(img, targets,category):
    names = category
...
def read_xml(xml_path,image_name,category):
...
def add_xml(xml_path,image_name,xml_save_path,add_object_xml,folders,image_save_name,img_w,img_h,category):
    xmlfile1 = xml_path + r'/' + image_name[:-4] + '.xml'
    tree1 = ET.parse(xmlfile1)
    doc = xml.dom.minidom.Document()
    root = doc.createElement("annotation")
    ...
    for obj in add_object_xml:
        obj_struct = {}
        obj_struct["name"] = category[int(obj[0])]
