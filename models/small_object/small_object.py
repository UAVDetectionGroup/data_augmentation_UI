...
def collect_small_object(xml_path,SOA_THRESH,Low_SOA_THRESH,category,objects):
    ...
    for xml_name in xmls:
        ...
        for obj in tree1.findall("object"):
            ...
            if issmallobject(name=obj_name, lower_thresh=SOA_THRESH, hight_thresh=Low_SOA_THRESH, h=bbox_h, w=bbox_w,
                             objects=objects):
...
def Samll_object_Augmentation(image_path,xml_path,image_save_path,xml_save_path,SOA_THRESH, SOA_PROB, SOA_COPY_TIMES, SOA_EPOCHS,Low_SOA_THRESH,category,objects):
    ...
    image_names = os.listdir(os.path.split(image_path)[0])
    # print('----------------------------')
    # print('小目标数据增强')
    ...
    return os.path.join(image_save_path,SMB_image_name )
